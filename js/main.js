...
(function(){
  ...
  /* ---------- Shared carousel autoplay (every single-photo popup) ----------
     One engine, reused by the Adventure, Wildlife, Food, and Wheels
     carousels: while open, advances to a random *other* photo every
     few seconds so browsing a set feels alive instead of a static
     first frame. Hovering the photo pauses it (resumes on mouseleave);
     any manual click — prev/next/dot — stops it for good, since at
     that point the person is driving, not the timer. */
  function createCarouselAutoplay({ length, onAdvance, hoverEl, intervalMs = 3200 }) {
    let timer = null, stopped = false, paused = false;
    function tick() {
      if (stopped || paused || length() < 2) return;
      onAdvance();
    }
    function start() { if (!stopped) { clearInterval(timer); timer = setInterval(tick, intervalMs); } }
    function stop() { stopped = true; clearInterval(timer); }
    const prefersReducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (hoverEl && !prefersReducedMotion) {
  ...
  window.createCarouselAutoplay = createCarouselAutoplay;
  ...
  /* Hobbies (Philately, Numismatics, Wildlife Photography) and My Toys —
     now rendered by js/hobbies.js as an interactive launcher + "The Garage",
     both sharing the #mediaDetailOverlay component defined in index.html. */
