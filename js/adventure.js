(function () {
  ...
  let activeEntry = null, activeMediaIndex = 0, lastFocusedIcon = null, autoplayCtrl = null;
  ...
  function renderMedia() {
    ...
    if (media.length > 1) {
      media.forEach((_, i) => {
        ...
        dot.addEventListener("click", () => { if (autoplayCtrl) autoplayCtrl.stop(); activeMediaIndex = i; renderMedia(); });
  ...
  function openCarousel(entry, iconEl) {
    activeEntry = entry;
    // Same reasoning as the Wildlife carousel: open on a random photo
    // from this activity rather than always the first one logged.
    activeMediaIndex = entry.media.length > 1 ? Math.floor(Math.random() * entry.media.length) : 0;
    lastFocusedIcon = iconEl;
    panelTitle.textContent = entry.title;
    renderMedia();
    overlay.classList.add("visible");
    overlay.querySelector(".adv-panel-close").focus();
    document.addEventListener("keydown", onOverlayKeydown);
    if (autoplayCtrl) autoplayCtrl.stop();
    autoplayCtrl = window.createCarouselAutoplay({
      length: () => activeEntry.media.length,
      onAdvance: () => {
        let next = Math.floor(Math.random() * activeEntry.media.length);
        if (next === activeMediaIndex) next = (next + 1) % activeEntry.media.length;
  ...
  function closeCarousel() {
    ...
    if (autoplayCtrl) { autoplayCtrl.stop(); autoplayCtrl = null; }
  }
