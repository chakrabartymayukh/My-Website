...
const TravelModule = (function () {
  ...
  // Picks a random item from an array's images (or the item itself if
  // just one) — used wherever a place/marker only shows one photo out
  // of several, so it isn't always the same one on every visit.
  function randomImage(images) {
    if (!images || !images.length) return "";
    return images.length > 1 ? images[Math.floor(Math.random() * images.length)] : images[0];
  }

  // Fisher–Yates shuffle, non-mutating — used to vary the display order
  // of a multi-photo reel each time it's opened, instead of always the
  // same (effectively chronological) order the data was logged in.
  ...
  function openPlaceDetail(placeId) {
    ...
    if (images.length > 1) {
      ...
      const itemsHtml = shuffled(images).map(itemHtml).join("");
  ...
  function initMap() {
    ...
    TRAVEL_DATA.allPlaces.forEach(place => {
      ...
      const photo = randomImage(place.images);
      const popupHtml = `
