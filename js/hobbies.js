...
(function () {
  ...
  if (hobbiesPanel) {
    ...
    let wcList = [], wcIndex = 0, wcLastFocused = null, wcAutoplay = null;
    ...
    function wcRenderMedia() {
      ...
      if (wcList.length > 1) {
        wcList.forEach((_, i) => {
          ...
          dot.addEventListener("click", () => { if (wcAutoplay) wcAutoplay.stop(); wcIndex = i; wcRenderMedia(); });
    ...
    function wcOpen(sp, iconEl) {
      wcList = bySpecies[sp];
      // Open on a random sighting rather than always the first (which,
      // since entries are logged roughly chronologically, would always
      // be the earliest encounter) — every visit to a species feels
      // fresh, and prev/next still cycle through all of them either way.
      wcIndex = wcList.length > 1 ? Math.floor(Math.random() * wcList.length) : 0;
      ...
      if (wcAutoplay) wcAutoplay.stop();
      wcAutoplay = window.createCarouselAutoplay({
        length: () => wcList.length,
        onAdvance: () => {
          let next = Math.floor(Math.random() * wcList.length);
          if (next === wcIndex) next = (next + 1) % wcList.length;
          wcIndex = next;
          wcRenderMedia();
        },
        hoverEl: wcMediaWrap
    ...
    function wcClose() {
      ...
      if (wcAutoplay) { wcAutoplay.stop(); wcAutoplay = null; }
    }
        ...
        ${speciesOrder.length ? `
          <div class="species-launcher" id="speciesLauncher">
            ${speciesOrder.map(sp => {
              ...
              const tilePhoto = list[Math.floor(Math.random() * list.length)].images[0];
              return `
                <button type="button" class="species-tile" data-species="${sp}">
                  <span class="species-tile-photo"><img src="${tilePhoto}" alt="${sp}" loading="lazy"></span>
  ...
  if (garagePanel && (D.wheelsAndWanderlust || D.myToys)) {
    ...
    garagePanel.innerHTML = `
      ...
      <div class="hob-launcher" id="vehicleLauncher">
        ${typeOrder.map(t => `
          <button type="button" class="hob-tile" data-type="${t}">
    ...
    /* ---- Vehicle carousel overlay (type → its vehicles) ----
       Same pop-up-with-prev/next/dots pattern as Adventure and
       Wildlife — clicking a vehicle type opens straight into it,
       on a random vehicle, rather than revealing a static grid
       first. */
    const vcOverlay = document.createElement("div");
    vcOverlay.className = "adv-overlay";
    vcOverlay.innerHTML = `
      <div class="adv-panel" role="dialog" aria-modal="true" aria-labelledby="vcPanelTitle">
        <button type="button" class="adv-panel-close" aria-label="Close">✕</button>
        <div class="adv-media-wrap" id="vcMediaWrap">
          <button type="button" class="adv-media-nav prev" aria-label="Previous vehicle">‹</button>
          <button type="button" class="adv-media-nav next" aria-label="Next vehicle">›</button>
        </div>
        <div class="adv-panel-body">
          <div class="adv-panel-title" id="vcPanelTitle"></div>
          <div class="adv-panel-caption" id="vcPanelCaption"></div>
          <div class="adv-panel-dots" id="vcPanelDots"></div>
        </div>
      </div>`;
    document.body.appendChild(vcOverlay);

    let vcList = [], vcIndex = 0, vcLastFocused = null, vcAutoplay = null;
    const vcMediaWrap = vcOverlay.querySelector("#vcMediaWrap");
    const vcTitle = vcOverlay.querySelector("#vcPanelTitle");
    const vcCaption = vcOverlay.querySelector("#vcPanelCaption");
    const vcDots = vcOverlay.querySelector("#vcPanelDots");
    const vcPrev = vcOverlay.querySelector(".adv-media-nav.prev");
    const vcNext = vcOverlay.querySelector(".adv-media-nav.next");

    function vcRenderMedia() {
      vcMediaWrap.querySelectorAll("img,.adv-empty-note").forEach(n => n.remove());
      if (!vcList.length) {
        const note = document.createElement("div");
        note.className = "adv-empty-note";
        note.textContent = "None in the collection yet.";
        vcMediaWrap.insertBefore(note, vcPrev);
        vcCaption.textContent = "";
      } else {
        const toy = vcList[vcIndex];
        const img = document.createElement("img");
        img.className = "adv-media-el";
        img.src = toy.photo;
        img.alt = toy.name;
        vcMediaWrap.insertBefore(img, vcPrev);
        vcCaption.textContent = [toy.name, toyLocationLine(toy), yearOnly(toy.visitDate)].filter(Boolean).join(" · ");
      }
      vcPrev.disabled = vcList.length < 2;
      vcNext.disabled = vcList.length < 2;
      vcDots.innerHTML = "";
      if (vcList.length > 1) {
        vcList.forEach((_, i) => {
          const dot = document.createElement("button");
          dot.type = "button";
          dot.className = i === vcIndex ? "active" : "";
          dot.setAttribute("aria-label", `Vehicle ${i + 1} of ${vcList.length}`);
          dot.addEventListener("click", () => { if (vcAutoplay) vcAutoplay.stop(); vcIndex = i; vcRenderMedia(); });
          vcDots.appendChild(dot);
        });
      }
    }
    function vcOpen(type, iconEl) {
      vcList = byType[type] || [];
      // Random vehicle first, same reasoning as Wildlife/Adventure.
      vcIndex = vcList.length > 1 ? Math.floor(Math.random() * vcList.length) : 0;
      vcLastFocused = iconEl;
      vcTitle.textContent = VEHICLE_LABELS[type] || "Vehicles";
      vcRenderMedia();
      vcOverlay.classList.add("visible");
      vcOverlay.querySelector(".adv-panel-close").focus();
      document.addEventListener("keydown", vcOnKeydown);
      if (window.PopupHistory) window.PopupHistory.notifyOpen(vcClose);
      if (vcAutoplay) vcAutoplay.stop();
      vcAutoplay = window.createCarouselAutoplay({
        length: () => vcList.length,
        onAdvance: () => {
          let next = Math.floor(Math.random() * vcList.length);
          if (next === vcIndex) next = (next + 1) % vcList.length;
          vcIndex = next;
          vcRenderMedia();
        },
        hoverEl: vcMediaWrap
    ...
    function vcClose() {
      vcOverlay.classList.remove("visible");
      document.removeEventListener("keydown", vcOnKeydown);
      if (vcLastFocused) vcLastFocused.focus();
      if (vcAutoplay) { vcAutoplay.stop(); vcAutoplay = null; }
    }
    function vcCloseUser() {
      vcClose();
      if (window.PopupHistory) window.PopupHistory.notifyClose();
    }
    function vcOnKeydown(e) {
      if (e.key === "Escape") vcCloseUser();
      else if (e.key === "ArrowLeft") vcStep(-1);
      else if (e.key === "ArrowRight") vcStep(1);
    }
    function vcStep(dir) {
      if (vcList.length < 2) return;
      if (vcAutoplay) vcAutoplay.stop();
      vcIndex = (vcIndex + dir + vcList.length) % vcList.length;
      vcRenderMedia();
    }
    vcOverlay.querySelector(".adv-panel-close").addEventListener("click", vcCloseUser);
    vcOverlay.addEventListener("click", e => { if (e.target === vcOverlay) vcCloseUser(); });
    vcPrev.addEventListener("click", () => vcStep(-1));
    vcNext.addEventListener("click", () => vcStep(1));

    let vcTouchStartX = null;
    vcMediaWrap.addEventListener("touchstart", e => { vcTouchStartX = e.touches[0].clientX; }, { passive: true });
    vcMediaWrap.addEventListener("touchend", e => {
      if (vcTouchStartX === null) return;
      const dx = e.changedTouches[0].clientX - vcTouchStartX;
      if (Math.abs(dx) > 40) vcStep(dx > 0 ? -1 : 1);
      vcTouchStartX = null;
    }, { passive: true });

    vTiles.forEach(tile => {
      tile.addEventListener("click", () => vcOpen(tile.dataset.type, tile));
    });
  }
})();
